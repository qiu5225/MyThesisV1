...
雾凇算法（RIME）是 Su 等人近期提出的元启发算法，该算法模拟不同风力条件下软霜与硬霜的生长过程，通过模拟雾凇的凝结、生长与升华机制实现优化搜索\overcite {su2023rime}。与蚁群优化算法\overcite {dorigo2007ant} 等经典元启发算法相比，RIME 算法具有参数设置简单、流程简洁的优势，同时具备优异的全局优化性能，在多个领域的优化问题中展现出潜力。
...
医学高维特征选择场景，Abu 等通过软硬搜索策略与改进贪心机制提升全局寻优能力、规避局部最优，针对特征选择的二元搜索空间特性，设计二进制改进版 mRIME 并选用四类传递函数，经 CEC 标准测试集与疾病诊断特征选择任务验证，该算法性能优于十种主流优化算法，为多领域复杂优化与特征选择问题提供了高效解决方案\overcite{abu2024advanced}。
Wang 提出融合混沌局部搜索、交互机制与改进硬霜冰穿刺机制的多策略 RIME 算法（MRIME），经三大公开入侵检测数据集与九个 UCI 数据集验证，该算法在各项评价指标、运行效率及数据适应性上均优于现有方法，在入侵检测特征选择领域具备良好应用潜力\overcite{wang2025multi}。
为解决高维数据下进化特征选择面临的 “维数灾难”，Wu 等人提出增强型 RIME 算法（ERIME），融合特征信息熵剪枝、DBSCAN 空间聚类粒子进化、粒子删除策略及马尔可夫链蒙特卡洛粒子生成策略，既降低高维计算负担、提升搜索效率，又保障种群粒子质量；经 26 个基准数据集（含 14 个高维数据集）与 7 种主流智能算法对比验证，该算法在收敛精度（较 RIME 提升 79.54\%）、分类误差率（较 RIME 降低 98.54\%）及特征筛选量（较 RIME 减少 85\%）上表现优异，高效破解了高维特征选择难题\overcite{wu2024feature}。
...
在实际应用领域，HHO 算法及其变体已取得多项突破：在通信领域，HHO-PAP 变体通过联合功率分配和布局优化，显著提升了无人机辅助可见光通信系统的总速率，成功解决了无线通信中的非凸优化难题\overcite{pham2020sum}。在医疗领域，HHO + 算法通过优化全 Elman 神经网络（FENN）的结构与参数，在心力衰竭监测任务中实现了 79\% 的敏感性、78\% 的准确率和 76\% 的特异性 ，其增强的搜索空间探索能力超越了传统优化方法，充分展现了自适应优化在医学诊断中的应用潜力\overcite{fetanat2021fully}。在医学影像处理领域，自适应 HHO（AHHO）算法通过动态调整探索开发平衡策略，在脑部 MRI 分割任务中树立了新标准，分割精度与效率均优于传统方法；此外，HHO 算法的优化能力还扩展到电磁应用领域，为无线网络中的智能反射表面优化提供了高效解决方案\overcite{xu2020intelligent}。在方法学层面，HHO 算法的改进研究也持续推进，包括结合仿生算法微调预训练模型、利用群体智能进行参数调优，以及通过 LLaMEA 等框架实现自动元启发演化等，这些研究不仅增强了 HHO 算法的理论基础，更拓宽了其在通信、医疗、计算社会科学等多领域的应用场景，充分验证了其应对复杂优化挑战的能力\overcite{van2024llamea,de2021improving,alazab2024effective}。


尽管 HHO 算法具备显著优势，但仍面临多项限制其在医学高维特征选择中广泛应用的挑战：首先是计算复杂度问题，尤其在处理基因测序、多模态临床数据等超高维任务时，群体智能算法的迭代搜索过程需要消耗大量计算资源，严重影响处理效率；其次，在 HHOSSA \overcite{issa2022efficient} 等混合改进方法中，多种技术的整合进一步增加了算法的处理时间与实现复杂度；此外，在离散化实现（如 DHHO）中，动态网络结构与离散搜索空间的适配性不足，可能导致算法性能衰减\overcite{khatri2023influence}。在高维任务中，算法的搜索效率与稳定性仍有较大提升空间。这些问题的解决对于提升 HHO 算法在医学高维特征选择等实际场景中的适用性至关重要，也是本文展开改进研究的核心出发点。

针对数据分析中冗余无关属性影响预测模型性能的问题，本文融合粒子群优化（PSO）与哈里斯鹰优化（HHO）算法，提出混合 PSO-HHO 包裹式特征选择方法，以欧氏距离 KNN 分类器筛选最优特征集；经 UCI 四个标准数据集验证，该混合算法分类准确率显著优于传统 PSO 与 HHO 算法\overcite{azeem2025new}。

针对假新闻检测存在的特征冗余、特征选择不佳、分类模型低效等问题，本文提出数据预处理、语言特征提取、基于哈里斯鹰优化（HHO）的特征选择与 CNN-BiLSTM 混合模型分类的四阶段检测方法，在四个公开数据集上取得优异准确率，检测性能优于现有主流方法\overcite{jain2025hybrid}。

针对 NASA MDP 软件缺陷数据集存在的类别高度失衡、特征冗余等难题，本文构建了融合高级预处理、二元哈里斯鹰优化（BHHO）特征选择与集成学习的分类框架：通过标准化、鲁棒缩放及 ADASYN 算法处理数据异常与类别不平衡问题，以 KNN 为评估器完成特征筛选，并对比测试了随机森林、支持向量机与 Stacking 集成模型；实验显示 Stacking 模型在部分数据集上性能最优，但统计检验表明其提升效果并非全部显著，该集成框架有效提升了软件缺陷预测性能，同时也为后续研究指明了方向\overcite{luthfi2025enhancing}。
