...
从各消融变体的性能表现来看，剔除能量自适应Cauchy-Gaussian扰动策略的IHHO2算法性能缺陷最为突出。该算法在优化过程中表现出明显的能力不足，尤其在F1和F7函数上，其求解平均值分别达到17568.08117与6230.323705，远差于完整IHHO算法。这一结果直接表明，能量自适应Cauchy-Gaussian扰动策略是提升算法全局搜索广度、帮助算法突破局部最优桎梏的核心关键模块。剔除经验互益策略的IHHO1仅在F5、F6函数上取得更优平均值，而剔除精英方差引导开发策略的IHHO3则在F2、F3、F8函数上表现更佳。该规律清晰揭示了各策略的功能侧重：经验互益策略主要作用于算法优化中期，能够有效强化种群个体间的信息交互与协同搜索能力；精英方差引导开发策略则聚焦于优化后期，精准提升算法的局部精细开发精度。
