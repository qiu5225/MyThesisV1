...
\cabstract{
特征选择对于提高机器学习模型性能至关重要。虽然元启发式算法被广泛用于特征选择，但其有效性常受计算开销和收敛到次优解的阻碍。本文提出一种改进的 RIME 算法（IRIME），它结合信息熵评估策略、随机重启机制和拉格朗日插值方法来克服这些局限。信息熵评估策略通过对特征动态加权增强种群多样性，随机重启机制防止过早收敛，拉格朗日插值方法通过逼近全局最优解加速收敛。在 CEC 2017 测试集上的大量实验表明，IRIME 优于七种领先算法，收敛速度更快且稳定性更高。在 8 个低到中等维度数据集和 4 个高维数据集上测试，该算法在显著减少冗余特征的同时实现了有竞争力的分类准确率。此外，消融研究证实了这三种策略的协同效应。结果凸显了 IRIME 在全局优化和特征选择方面的鲁棒性，为特征选择提供了实用工具。
}
\ckeywords{元启发算法，RIME，信息熵，拉格朗日插值，特征选择}

\eabstract{
Feature selection is crucial for improving model performance in machine learning. While meta-heuristic algorithms are widely employed for feature selection, their effectiveness is frequently hampered by computational overhead and convergence to suboptimal solutions. We propose an improved RIME algorithm (IRIME) in this work. It combines information entropy evaluation strategy, random restart mechanism, and Lagrange interpolation method to overcome these limitations. The information entropy evaluation strategy enhances population diversity by dynamically weighting features, while the random restart mechanism prevents premature convergence and the Lagrange interpolation method accelerates convergence by approximating global optima. Extensive experiments on the CEC 2017 test suite, IRIME's outperforms seven leading algorithm, with faster convergence and higher stability. Tested on 8 datasets with low to moderate dimensionality and 4 high-dimensional datasets, our algorithm achieves competitive classification accuracy while significantly reducing redundant features. Furthermore, the ablation studies confirm the synergistic effect of the three strategies. The results highlight IRIME's robustness in global optimization and feature selection, offering a practical tool for feature selection.
}
% 英文文关键词(每个关键词之间用,分开, 最后一个关键词不打标点符号。)
\ekeywords{meta-heuristic, RIME, information entropy, Lagrange interpolation, feature selection}
...
\section{本文组织结构}
本文组织结构如下：第一章节阐述相关工作；第二章节节介绍特征选择问题与原始 RIME 算法；第三章节详细说明 IRIME 算法及其改进策略；第四章节展示 CEC2017 测试集的完整实验结果并分析；第五章节节给出算法在特征选择任务中的应用结果与详细分析；最后在第六章节总结全文工作，并展望未来研究方向。
\chapter{相关背景知识介绍}
\section{特征选择问题}
\subsection{特征选择问题描述}
...
\section{原始RIME算法}
\subsection{RIME概述}
RIME 受雾凇形成这一自然现象的启发。该算法模拟了不同环境条件下水汽的凝结过程，如先前的研究（H. Su 等人，2023）中详细描述的那样。RIME 框架由四个关键阶段组成：（1）种群初始化，（2）软雾凇搜索，（3）硬雾凇穿刺，以及（4）正向贪婪选择。这些阶段共同使算法能够有效地探索和利用解空间。图 1 提供了 RIME 工作流程的可视化表示，而相应的算法步骤在算法 1 中进行了概述。
...
\subsection{正贪婪选择机制}
在 RIME 算法中，正贪婪选择机制通过直接的适应度比较确定性地选择更优解。这种方法只接受能提高适应度的更新，确保种群持续改进。然而，这种机制在优化后期表现出固有的收敛到局部最优的趋势。后续 4.2 节提出的随机重启机制有效地缓解了这一局限性。
\chapter{IRIME算法}
本研究通过整合信息熵评估策略、拉格朗日插值法和随机重启机制这三个关键策略，对 RIME 算法进行了改进，得到 IRIME 算法。信息熵的引入增强了各智能体之间的信息交换，拉格朗日插值法加快了收敛速度，随机重启机制有助于保持解的多样性。这三种策略的协同组合使 IRIME 在解决特征选择等复杂问题时表现出更强的适应性和优化能力，在收敛速度和解的质量方面均优于原始算法。算法 4 中提供的伪代码详细说明了 IRIME 的实现步骤，同时附图 2 直观展示了该算法的工作流程。
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{figures//Flowchart/IRIME2flowchart.png}
    \caption{Flowchart of IRIME}
...
\centering
\small
\caption{Parameter configurations for the competing algorithms on benchmark}
\begin{tabular}{ll}
\toprule
...
\centering
\footnotesize
\caption{Details of CEC 2017 test suite functions}
...
\begin{figure}[htbp]
    ...
    \caption{Convergence curves on 12 benchmark functions (30D).}
...
\begin{figure}[htbp]
    ...
    \vspace{0.5cm}
    \includegraphics[width=0.5\textwidth]{figures/Convergence/dim30/legend.pdf}
    \caption{Convergence curves on 12 benchmark functions (50D).}
...
\begin{figure}[htbp]
    ...
    \caption{The Friedman test result of nine algorithms (30D).}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Box diagram on 12 benchmark functions (30D).}
    \label{fig:box-dim30}
\end{figure}
% 50维箱型图
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Box diagram on 12 benchmark functions (50D).}
    \label{fig:box-dim50}
\end{figure}
...
\begin{table}[htbp]
  \centering
  \caption{Comparative results of nine algorithms on CEC 2017 functions (30D).}
...
\begin{table}[htbp]
  \centering
  \caption{Comparative results of nine algorithms on CEC 2017 functions (50D).}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
    \begin{tabular}{ccccccccccc}
    \toprule    
...
\begin{table}[htbp]
  \centering
  \caption{Comparative results of IRIME and its variants on CEC 2017 functions (30D)}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
...
\begin{table}[htbp]
  \centering
  \caption{Comparative results of IRIME and its variants on CEC 2017 functions (50D)}
...
\begin{figure}[htbp]
    ...
    \caption{Friedman test result of IRIME and its variants}
    \label{fig:Ftest-ablation}
\end{figure}
\begin{table}[htbp]
  \centering
  \small
  \caption{Details of IRIME variants ('Y' = strategy used ; 'N' = not used) }
  \begin{tabular}{lcccc}
    \toprule
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Convergence curves on 12 datasets.}
    \label{fig:fs-cov}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Comparative accuracy histograms on 12 datasets.}
    \label{fig:fs-acc}
\end{figure}

\begin{table}[htbp]
  \centering
  \caption{The fitness value results on 12 datasets}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
...
\begin{table}[htbp]
  \centering
  \caption{The accuracy results on 12 datasets}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
    \begin{tabular}{cccccccccc}
    \toprule    
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Comparative feature size histograms on 12 datasets.}
...
\chapter{总结与展望}
在本研究中，我们提出了一种改进型 RIME 算法（IRIME），旨在解决原始 RIME 算法存在的局限性。具体而言，该改进方案通过整合三大核心创新机制实现性能突破：（1）信息熵评估策略通过量化搜索过程中的信息不确定性，动态平衡算法的探索（Exploration）与利用（Exploitation）能力，避免算法过早陷入局部最优或搜索效率低下的困境；（2）随机重启机制当算法检测到搜索进程陷入停滞状态时，自动触发随机初始化重启，重新开辟搜索路径，有效规避局部最优解的束缚；（3）拉格朗日插值法通过构建光滑的插值函数拟合搜索空间的目标函数分布，为算法提供更精准的搜索方向指引，从而显著提升收敛速度。
为全面验证 IRIME 算法的性能优势，我们在两大核心任务场景中开展了系统性实验：CEC 2017 基准测试集与特征选择任务。在 CEC 2017 测试集上，无论面对低维还是高维优化问题，IRIME 均展现出卓越的全局优化能力不仅收敛速度远超原始 RIME 及其他对比算法，还能获得更高的求解精度，充分验证了其在复杂优化场景中的适应性。在特征选择任务中，IRIME 能够精准识别数据集中的关键特征，有效剔除冗余信息，进而显著提升后续分类模型的性能；多组数据集上的实验结果均表明，其特征选择效果与分类精度均优于现有主流对比算法，凸显了算法在实际数据处理任务中的实用价值。
此外，我们通过消融实验（Ablation Studies）进一步验证了三大创新机制的有效性：单独移除任一机制后，算法的优化性能、收敛速度或特征选择精度均出现明显下降，而三者的有机结合则实现了 “1+1+1>3” 的协同效应，显著增强了算法的整体性能。这一结果充分证明了各创新模块设计的合理性与必要性，为算法的后续改进提供了坚实的实验支撑。
展望未来，相关研究可从两方面进一步推进：一方面，可通过优化算法的计算流程、引入并行计算等方式，进一步提升 IRIME 的计算效率，以适应更大规模数据或更复杂优化问题的需求；另一方面，可探索融合更多先进搜索策略，拓展算法在多模态优化问题中的性能边界，此类问题因存在多个最优解而极具挑战性，IRIME 的现有框架为相关改进提供了良好基础。同时，IRIME 在工程实践中具有广阔的应用前景，尤其适用于图像处理（如图像分割、特征提取）、医疗诊断（如疾病风险预测、医学影像分析）等对优化精度与效率均有严格要求的领域。其高效处理优化问题与特征选择任务的双重能力，充分彰显了算法的通用性与实用价值，为解决实际工程中的复杂问题提供了新的技术思路。
\newpage
