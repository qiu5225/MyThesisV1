...
元启发式算法凭借其天然的全局优化特性，与特征选择任务的核心需求高度适配\overcite{brezovcnik2018swarm}。元启发式算法能够通过模拟自然群体行为高效筛选出低冗余、高判别力的特征子集。为实现复杂高维数据的有效处理，科研人员围绕优化算法的搜索策略展开了大量改进研究，通过融合多策略、增强种群多样性、动态平衡探索与开发过程等方式提升算法优化性能，进而提出了一系列基于元启发式算法的特征选择方法\overcite{aly2025hybrid,chen2025novel,cai2025dynamic}。

针对医学高维数据集的特征选择任务，现有元启发式算法面临的挑战更为突出\overcite{kamal2024comprehensive}。医学数据的高维性使得搜索空间呈指数级爆炸式增长，加剧了数据稀疏性与基于距离的求解难题；同时，医学特征往往具有强相关性、非线性耦合等特点，导致算法更易陷入局部最优，难以挖掘到真正具有临床意义的核心特征子集\overcite{jia2022feature}。具体而言，元启发式算法在医学高维特征选择中面临双重核心挑战：一是迭代过程中解的多样性持续衰减，削弱全局优化能力，无法突破医学特征空间中的局部最优陷阱；二是算法性能对人工调优的参数高度敏感，导致性能波动较大，而医学数据的异质性进一步放大了参数依赖性带来的负面影响，降低了算法在不同医学场景下的泛化能力\overcite{huang2025q}。
...
近年来，启发式算法已成为机器学习领域的核心优化工具，尤其适用于求解具备大解空间与 NP 难特性的复杂优化问题\overcite {karimi2022machine}。这类算法依托创新搜索策略生成近似最优解，在求解效率与场景适应性上普遍优于传统优化方法。按照算法的启发来源划分，启发式算法可归为四大类别\overcite {ss2022nature}：进化算法\overcite{liang2025survey}、群智能算法\overcite{priyadarshi2025evolution}、物理启发算法与人类行为启发算法\overcite {zhao2024triangulation}。
...
雾凇算法（RIME）是 Su 等人近期提出的元启发式算法，该算法模拟不同风力条件下软霜与硬霜的生长过程，通过模拟雾凇的凝结、生长与升华机制实现优化搜索\overcite {su2023rime}。与哈里斯鹰优化算法\overcite {heidari2019harris}、蚁群优化算法\overcite {dorigo2007ant} 等经典元启发式算法相比，RIME 算法具有参数设置简单、流程简洁的优势，同时具备优异的全局优化性能，在多个领域的优化问题中展现出潜力。
...
哈里斯鹰优化算法（Harris Hawk Optimization, HHO），是一种模仿哈里斯鹰（Harris's hawks）合作狩猎行为的自然启发式元启发式优化技术，该算法在2019年由 Heidari 等人于提出\overcite{heidari2019harris}。该算法属于群体智能方法范畴，通过模拟哈里斯鹰的猎物包围、突袭及动态协作策略，构建了自适应的搜索机制，其核心优势在于能够动态平衡全局探索与局部开发过程，有效应对传统基于导数的优化器易陷入局部最优的问题，在复杂优化任务中展现出显著优越性。
...
通常情况，元启发式优化算法具有一种贪婪选择机制，在每次更新后都会替换并记录最佳适应度值和最佳个体。其典型思路是将个体的更新适应度值与全局最优解进行比较；如果更新后的值优于当前的全局最优解，则替换最优适应度值，并将该个体记录为最优解。这种操作的优点在于简单且快速，但它对种群的探索（exploration）和开发（exploitation）并无实质帮助，仅仅起到记录的作用。
...
\section{引言}
在实际工程优化领域，医学特征选择是一类典型的复杂优化问题，其核心在于从高维医学数据中筛选出最具判别力的特征子集，在降低计算复杂度的同时，提升疾病诊断与分类的准确率。这类问题往往具有维度高、特征冗余度大、数据分布复杂等特点，对优化算法的探索能力、收敛速度及稳定性提出了更高要求。现有元启发式算法应用于医学特征选择时，常出现特征筛选效率低、分类精度不稳定、难以适配高维医学数据集等问题，亟需一种性能更优的改进型优化算法加以解决。
...
本章后续内容将按以下逻辑展开：首先，详细阐述IRIME算法的核心改进策略与整体实现流程，明确各策略的作用机制及算法时间复杂度；其次，通过CEC 2017基准函数实验，将IRIME与多种先进元启发式算法进行对比，从收敛速度、解质量、稳定性等维度验证其理论优化性能；再次，将IRIME应用于12个不同维度的医学数据集特征选择问题，验证其在实际工程场景中的应用价值；最后，通过消融实验剖析各改进策略的作用的重要性，并对本章研究内容进行总结，为后续算法的进一步优化与拓展奠定基础。
...
\subsection{基准函数对比实验}
在本节中，我们通过在 CEC 2017 测试集上进行广泛的实验，将所提出的 IRIME 算法与八个先进的竞争对手进行了评估。所有对比算法的参数配置列于表 1。研究涵盖三个关键分析维度：(1) 通过迭代曲线进行收敛行为分析，(2) 通过箱线图可视化进行解的稳定性评估，(3) 使用弗里德曼检验进行非参数统计验证。所有实验均在严格控制的条件下进行。
...
\centering
\small
\bicaption{CEC 2017 测试套件函数的详细信息}{Details of CEC 2017 test suite functions}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \bicaption{12 个基准函数上的箱线图 (30D)}{Boxplots on the 12 benchmark functions (30D)}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \bicaption{12 个基准函数上的箱线图 (50D)}{Boxplots on the 12 benchmark functions (30D)}
...
\begin{table}[htbp]
  \centering
  \bicaption{九种算法在 CEC 2017 函数上的对比结果 (30D)}{Comparison results of nine algorithms on CEC 2017 function (30D)}
...
\begin{table}[htbp]
  \centering
  \bicaption{九种算法在 CEC 2017 函数上的对比结果 (50D)}{Comparison results of nine algorithms on CEC 2017 function (50D)}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
    \begin{tabular}{ccccccccccc}
    \toprule    
...
\begin{table}[htbp]
  \centering
  \bicaption{IRIME消融实验结果 (50D)}{Results of IRIME ablation experiments (50D)}
...
\subsection{特征选择应用}
本节旨在通过选择医学特征选择（FS）问题作为案例，提升IRIME在工程问题中的清晰度和有效性，以评估其优化性能。为确保全面评估，我们将所提方法与七种先进的元启发式算法进行比较：RIME、AOA、BWO、WOA、ROA、SCA和COA。
\subsubsection{实验准备}
所有参与算法的参数设置均统一，种群规模为N=30，每次运行的最大计算预算为3000次迭代。算法的参数设置详情严格按照表 \ref{tb:FS-alg-parameters-2} 中所列的进行实验。本研究所用的所有实验数据集均来自医学领域。数据集包括 8 个中低维度和 4 个高维度数据集，涵盖多样的数据特性，以严格测试算法的可扩展性和鲁棒性。这些数据集的详细规格（如样本量、特征维度和类别分布）见表 \ref{tb:IHHO-datasets-info}。此分层数据设计旨在验证IRIME在不同问题复杂度下的表现，同时保持其在医学应用中的实际相关性。实验的软、硬件则与前文一直，严格按照 \ref{sec:exp-settings} 节中的软硬件环境进行实验。
%算法参数设置表2
\begin{table}[htbp]
\centering
\small
...
\begin{table}[htbp]
  \centering
  \bicaption{所用医学数据集概览}{Overview of the medical dataset used}
  ...
  \begin{tabular}{cccc}
    ...
    Colon       & 60      & 2000     & 2       \\
    Leukemia    & 72      & 7129     & 2       \\
    Leukemia\_3c& 72      & 7129     & 3       \\
    Leukemia\_4c& 72      & 7129     & 4       \\
    Lung        & 181     & 12533    & 2       \\
    Ovarian     & 253     & 15154    & 2       \\
    \bottomrule
  \end{tabular}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    \bottomrule
  \end{tabular}
  \caption*{Note: Y = 使用策， N = 未使用} % 说明放在这里
...
\begin{table}[htbp]
  \centering
  \bicaption{IRIME及其变体在CEC 2017函数上的比较结果（30D）}{Comparative results of IRIME and its variants on CEC 2017 functions (30D)}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \tiny
    \begin{tabular}{rrrrrrccrrrr}
	\toprule
...
\begin{table}[htbp]
  \centering
  \bicaption{IRIME及其变体在CEC 2017函数上的比较结果（50D）}{Comparative results of IRIME and its variants on CEC 2017 functions (50D)}
    \setlength{\tabcolsep}{3pt}
    \tiny
    \begin{tabular}{rrrrrrccrrrr}
    \toprule
...
\end{equation}
其， $\sigma$ 为 Gaussian 扰动的初始尺度参数。
之， 使用公式 (23) 构造能量自适应 Cauchy-Gaussian 扰动候选解。
\begin{equation}
...
\begin{figure}[p]
    \centering
    % 第一张图
    \includegraphics[width=0.9\linewidth]{figures/HHO/cov_cec2020.pdf}
    \bicaption{CEC 2020 对比实验收敛曲线图 (10D)}{Convergence curve of CEC 2020 comparative experiments (10D)}
    \label{fig:IHHO-cov-dim10}

    \vspace{1em} % 图间距

    % 第二张图
    \includegraphics[width=0.9\linewidth]{figures/HHO/cov_cec2020_30.pdf}
    \bicaption{CEC 2020 对比实验收敛曲线图 (30D)}{Convergence curve of CEC 2020 comparative experiments (30D)}
...
\begin{table}[htbp]
  \centering
  \small
  \bicaption{各算法在 CEC 2020 的对比实验结果(10D)}{Comparative experimental results of each algorithm in CEC 2020 (10D)}
...
\begin{table}[htbp]
  \centering
  \small
    \bicaption{各算法在 CEC 2020 的对比实验结果(30D)}{Comparative experimental results of each algorithm in CEC 2020 (30D)}
    \begin{tabular}{ccrrrrrr}
        \toprule
    函数    & 类型    & \multicolumn{1}{c}{IHHO} & \multicolumn{1}{c}{HHO} & \multicolumn{1}{c}{BWO} & \multicolumn{1}{c}{AOA} & \multicolumn{1}{c}{SCA} & \multicolumn{1}{c}{RSA} \\
    \midrule
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Ftest/IHHO_DIM10_Ftest.pdf} % 调整宽度为适合您文档的值
    \bicaption{IHHO与其他5种算法在CEC 2020 测试集上的平均排名（10D）}{Average ranking of IHHO with other five algorithms on the CEC 2020 test set(10D)}
    \label{fig:IHHO-Ftest-DIM10}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Ftest/IHHO_DIM30_Ftest.pdf} % 调整宽度为适合您文档的值
    \bicaption{IHHO与其他5种算法在CEC 2020 测试集上的平均排名（30D）}{Average ranking of IHHO with other five algorithms on the CEC 2020 test set (30D)}
...
\begin{table}[htbp]
  \centering
  \bicaption{各算法弗里德曼排名结果(10D)}{Friedman ranking results for each algorithm (10D)}
...
\begin{table}[htbp]
  \centering
  \bicaption{各算法弗里德曼排名结果(30D)}{Friedman ranking results for each algorithm (30D)}
    \begin{tabular}{ccccccc}
    \toprule
    函数    & IHHO  & HHO   & BWO   & AOA   & SCA   & RSA \\
    \midrule
