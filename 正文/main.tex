...
\section{雾凇算法}
\subsection{算法概述}
雾凇算法受雾凇形成这一自然现象的启发\overcite{su2023rime}。该算法模拟了不同环境条件下水汽的凝结过程。RIME 框架由四个关键阶段组成：（1）种群初始化，（2）软雾凇搜索，（3）硬雾凇穿刺，以及（4）正向贪婪选择。这些阶段共同使算法能够有效地探索和利用解空间。\ref{fig:RIME-Flowchart} 提供了 RIME 工作流程的可视化表示，而相应的算法步骤在算法 \ref{alg:RIME} 中进行了概述。
...
其中 ${r}_{2} \in  \left( {0,1}\right)$ 是一个均匀分布的随机变量，它与 $E,{242}$ 共同控制粒子凝聚（位置更新）。完整的软雾搜索方法详见算法\ref{alg:soft-rime-search}。
\begin{equation}
    E = \sqrt{(\frac {t}{T})}
\label{eq:E}
\end{equation}
...
\section{哈里斯鹰算法}
\subsection{算法概述}
哈里斯鹰优化算法 (Harris Hawks Optimization, HHO) 是 Heidari 等人提出的一种新的群智能优化方法, 它的理论依据来自于自然界中哈里斯鹰群体捕猎行为的复杂过程\overcite{heidari2019harris}。实际捕猎中, 它会表现出侦察、围捕、追踪、 攻击等各种层次的合作模式，依靠不断改变的捕食行为一步步逼近目标。HHO 把这一自然现象用数学建模的方式表达出来, 在鹰群追逐猎物的时候, 用互动博弈的方式来引导种群个体在搜索空间里找到最优解并达到精准搜寻的目的。
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{figures//Flowchart/IRIME流程图_横板1.png}
...
\subsubsection{实验准备}
本实验所选取的算法及算法参数设置如\ref{tb:alg-parameters-1}表所示。本实验所涉及的CEC 2017数据集的相关信息在\ref{tb:CEC2017-functions}表中详细展示。实验的软、硬件环境在 \ref{sec:exp-settings} 节进行了详细介绍。
...
\begin{table}[htbp]
  \centering
  \caption{IRIME消融实验结果 (30D)}{Results of IRIME ablation experiments (30D)}
    \setlength{\tabcolsep}{3pt} % 减少列间距
    \scriptsize
...
为确保全面评估IRIME的应用价值，我们将所提方法与七种先进的元启发式算法进行比较：RIME、AOA、BWO、WOA、ROA、SCA和COA。所有算法的参数设置均统一，种群规模为N=30，每次运行的最大计算预算为3000次迭代，具体详见表8。本研究所用的所有实验数据集均来自医学领域。数据集包括 8 个中低维度和 4 个高维度数据集，涵盖多样的数据特性，以严格测试算法的可扩展性和鲁棒性。这些数据集的详细规格（如样本量、特征维度和类别分布）见\ref{tb:datasets-information}。此分层数据设计旨在验证IRIME在不同问题复杂度下的表现，同时保持其在医学应用中的实际相关性。实验的软、硬件则与前文一直，在 \ref{sec:exp-settings} 节进行了详细介绍。
...
\begin{table}[htbp]
    \centering
    \bicaption{Overview of datasets}{数据集概览}
    \small
      \begin{tabular}{lrrrr}
        \toprule
...
\centering
\small
\bicaption{特征选择实验中各算法参数设置}{Parameters setting for
algorithms used in FS}
\begin{tabular}{ll}
\toprule
\textbf{Algorithms} & \textbf{Other parameters} \\
...
\begin{table}[htbp]
    \centering
    \bicaption{数据集概览}{Overview of the dataset}
    \small
      \begin{tabular}{lrrrr}
        \toprule
      Dataset Name & \multicolumn{1}{l}{Samples} & \multicolumn{1}{l}{Features} & \multicolumn{1}{l}{Classes} \\
...
为验证IHHO算法的有效性与优越性，本章设计了多组对比实验与应用验证：以CEC2020基准测试函数为对象，将IHHO与原始HHO及其他先进元启发式算法（白鲸优化算法BWO、算术优化算法AOA、正余弦优化算法SCA、爬行动物搜索算法RSA）进行对比，从最优值、平均值、标准差及收敛曲线等维度评估算法的优化性能；通过消融实验验证所提三种改进策略的独立作用与协同效果；采用正交实验分析算法关键参数对优化性能的影响，确定最优参数组合；进一步将IHHO算法应用于高维生物医学数据集的特征选择问题，验证其在实际复杂问题中的应用价值。本章的研究成果旨在克服原始HHO算法的固有缺陷，为连续优化与高维特征选择等相关领域提供更高效、更稳定的优化解决方案，同时丰富元启发式算法的改进思路与应用场景。
...
\begin{landscape}
    \begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{figures/HHO/cov_cec2020.pdf} % 调整宽度为适合您文档的值
    \bicaption{CEC 2020 对比实验收敛曲线图}{Convergence curve of CEC 2020 comparative experiments}
    \label{fig:IHHO-cov}
\end{figure}
\end{landscape}
...
\begin{table}[htbp]
  ...
  \bicaption{各算法弗里德曼排名结果(10D)}{Friedman ranking results for each algorithm (10D)}
    ...
    \midrule
    CEC1  & 1     & 2.03  & 5     & 3.93  & 3.03  & 6 \\
    CEC2  & 1     & 2.13  & 4.97  & 2.93  & 3.97  & 6 \\
    CEC3  & 1     & 2.7   & 4.23  & 4.67  & 2.4   & 6 \\
    CEC4  & 3.42  & 3.42  & 3.42  & 3.42  & 3.92  & 3.42 \\
    CEC5  & 1.33  & 2.2   & 5     & 3.6   & 2.87  & 6 \\
    CEC6  & 1.13  & 2.83  & 4.23  & 4.77  & 2.03  & 6 \\
    CEC7  & 1     & 3.1   & 5     & 2.23  & 3.67  & 6 \\
    CEC8  & 1.1   & 2.1   & 4     & 5     & 2.8   & 6 \\
    CEC9  & 2.03  & 4.43  & 1.43  & 4.2   & 3.1   & 5.8 \\
    CEC10 & 1.37  & 1.67  & 4.97  & 3.97  & 3.03  & 6 \\
    \bottomrule
    \end{tabular}%
  \label{tab:addlabel-X}%
\end{table}%
...
\begin{table}[htbp]
  ...
  \bicaption{各算法弗里德曼排名结果(30D)}{Friedman ranking results for each algorithm (30D)}
    ...
    函数    & IHHO  & HHO   & BWO   & AOA   & SCA   & RSA \\
    \midrule
    CEC1  & 1     & 2     & 5     & 4.23  & 3     & 5.77 \\
    CEC2  & 1     & 2     & 5.13  & 3     & 5.87  & 4 \\
    CEC3  & 1     & 2.97  & 5     & 4     & 2.03  & 6 \\
    CEC4  & 3.47  & 3.47  & 3.47  & 3.47  & 3.67  & 3.47 \\
    CEC5  & 1     & 2     & 5     & 3.67  & 3.33  & 6 \\
    CEC6  & 1     & 2.03  & 5.13  & 3.67  & 3.5   & 5.67 \\
    CEC7  & 1     & 2     & 5     & 3.07  & 3.93  & 6 \\
    CEC8  & 1     & 2.07  & 3.87  & 4.6   & 4.97  & 4.5 \\
    CEC9  & 1     & 3.93  & 4.6   & 5.97  & 2.03  & 3.47 \\
    CEC10 & 1.03  & 1.97  & 4.27  & 4.77  & 3     & 5.97 \\
    \bottomrule
    \end{tabular}%
  \label{tab:addlabel-30}%
\end{table}%
...
\begin{table}[htbp]
  ...
  \bicaption{各算法在 CEC 2020 的对比实验结果(10D)}{Comparative experimental results of each algorithm in CEC 2020 (10D)}
    ...
    \midrule
    \multirow{3}[0]{*}{CEC1} & best   & \textbf{101.7720894} & 64448.57196 & 2420526511 & 9442.890488 & 121594103 & 5369474730 \\
    ...
    \multirow{3}[0]{*}{CEC2} & best   & \textbf{1267.588189} & 1411.895954 & 1987.85122 & 1570.751564 & 1669.634083 & 2153.475578 \\
    ...
    \multirow{3}[0]{*}{CEC3} & best   & \textbf{721.9906692} & 743.9547336 & 765.632476 & 740.9436746 & 746.5344821 & 781.0089749 \\
    ...
    \multirow{3}[0]{*}{CEC4} & best   & \textbf{1900} & \textbf{1900} & \textbf{1900} & \textbf{1900} & \textbf{1900} & \textbf{1900} \\
    ...
    \multirow{3}[0]{*}{CEC5} & best   & \textbf{2088.046571} & 2510.295874 & 17101.88428 & 4112.215234 & 4934.424807 & 150984.8829 \\
    ...
    \multirow{3}[0]{*}{CEC6} & best   & \textbf{1601.818907} & 1601.985459 & 1732.40058 & 1748.322331 & 1631.825291 & 1900.937955 \\
    ...
    \multirow{3}[0]{*}{CEC7} & best   & \textbf{2121.415836} & 2459.823994 & 4475.802391 & 2449.940027 & 3764.229937 & 7787.135327 \\
    ...
    \multirow{3}[0]{*}{CEC8} & best   & 2301.263718 & 2305.232511 & 2341.023712 & 2420.282114 & \textbf{2276.520994} & 2707.135762 \\
    ...
    \multirow{3}[0]{*}{CEC9} & best   & \textbf{2500} & 2500.520498 & 2592.113845 & 2559.470956 & 2557.728937 & 2697.579131 \\
    ...
    \multirow{3}[0]{*}{CEC10} & best   & \textbf{2897.762447} & 2897.913084 & 3048.739514 & 2906.442449 & 2920.546914 & 3140.093286 \\
...
\begin{table}[htbp]
  ...
  \small
    ...
    函数    & 类型    & \multicolumn{1}{c}{IHHO} & \multicolumn{1}{c}{HHO} & \multicolumn{1}{c}{BWO} & \multicolumn{1}{c}{AOA} & \multicolumn{1}{c}{SCA} & \multicolumn{1}{c}{RSA} \\
    \midrule
    \multirow{3}[0]{*}{CEC1} & best   & \textbf{159.1680803} & 4218472.6 & 39446670411 & 32362103001 & 9336541254 & 30205319503 \\
    ...
    \multirow{3}[0]{*}{CEC2} & best   & \textbf{3601.786982} & 3661.14317 & 7347.207346 & 4758.860926 & 7422.465947 & 6602.807251 \\
    ...
    \multirow{3}[0]{*}{CEC3} & best   & \textbf{928.5032622} & 1061.206197 & 1207.514762 & 1155.410049 & 1054.518949 & 1292.096779 \\
    ...
    \multirow{3}[0]{*}{CEC5} & best   & \textbf{68122.31073} & 124875.9138 & 11856236.52 & 890115.4362 & 2179389.19 & 16764122.71 \\
    ...
    \multirow{3}[0]{*}{CEC6} & best   & \textbf{1824.967983} & 2022.860321 & 3484.487919 & 2274.736871 & 2830.948042 & 3376.583509 \\
    ...
    \multirow{3}[0]{*}{CEC7} & best   & \textbf{23548.03688} & 54335.95964 & 1222443.945 & 55505.82944 & 460118.9906 & 3612453.212 \\
    ...
    \multirow{3}[0]{*}{CEC8} & best   & \textbf{2300} & 2317.380141 & 6611.821403 & 6600.114085 & 3377.713676 & 5959.043729 \\
    ...
    \multirow{3}[0]{*}{CEC9} & best   & \textbf{2904.45271} & 3101.48786 & 3325.762439 & 3416.004854 & 3108.322009 & 3256.634917 \\
    ...
    \multirow{3}[0]{*}{CEC10} & best   & \textbf{2883.586413} & 2884.941735 & 3862.64409 & 3400.093889 & 3090.315184 & 3851.998478 \\
...
\subsection{消融实验}
为验证所提三种改进策略的实际作用，本文在CEC2020测试函数集上进行了消融实验。实验设置与前文保持一致，仅删除对应策略以保证比较公平性。其中，IHHO为包含三种改进策略的完整算法。IHHO1表示去除经验互益策略。IHHO2表示去除能量自适应Cauchy-Gaussian扰动策略。IHHO3表示去除精英方差引导开发策略。
从整体结果来看，完整IHHO仍表现出较好的综合竞争力。以平均值为主要指标，IHHO在CEC1、CEC4、CEC7和CEC10上取得最优结果，在CEC3和CEC8上也保持较强竞争力。以最优值来看，IHHO在CEC1、CEC3、CEC4、CEC7、CEC8、CEC9和CEC10上取得最优或并列最优结果。说明三种策略协同后，算法在全局探索、局部开发和结果稳定性之间实现了较好的平衡。
从各消融版本看，IHHO2有所不足。特别是在CEC1和CEC7上平均值分别为17568.08117和6230.323705，说明能量自适应Cauchy-Gaussian扰动策略是提升全局搜索能力和跳出局部最优能力的关键因素。IHHO1在CEC5和CEC6上取得更优平均值，IHHO3在CEC2、CEC3和CEC8上表现较好，表明经验互益策略主要增强中期协同搜索能力，而精英方差引导开发策略主要提升后期开发精度。总体而言，三种策略具有较强互补性，完整IHHO因此获得了更均衡的优化性能。
...
表格8中是种群大小N=30，dim=30，每个算法在IEEE CEC2020独立运行30次获得的统计结果。由表8的各测试函数的实验结果可以看出，不同参数组合对IHHO的求解性能存在较为明显的影响，说明参数设置对算法的搜索行为具有直接作用。从具体结果来看，各组参数在不同函数上的优势并不完全一致。例如，IHHO6在CEC1上取得了较好的平均值，IHHO7在CEC2上表现较优，IHHO4在CEC3上更具优势，IHHO8在CEC7上取得了较好的结果，IHHO5则在CEC10上表现突出。这表明，不同测试函数对搜索步长、精英保留比例以及初始扰动强度的要求并不相同，单一参数组合难以在所有问题上同时保持绝对优势。
...
\begin{landscape}
  ...
  \bicaption{IHHO参数敏感度实验结果表}{Table of experimental results of IHHO parameter sensitivity}
    ...
    \midrule
    \multirow{3}[0]{*}{CEC1} & best   & 106.5  & 104.1  & \textbf{100.0 } & 128.2  & 111.5  & 241.9  & 106.0  & 104.4  & \textbf{100.6 } & 102.0  \\
    ...
    \multirow{3}[0]{*}{CEC2} & best   & 1270.1  & 1241.4  & 1241.1  & \textbf{1230.3 } & 1245.2  & 1232.0  & 1153.5  & \textbf{1103.8 } & 1258.4  & 1161.9  \\
    ...
    \multirow{3}[0]{*}{CEC3} & best   & 723.2  & 724.7  & \textbf{718.3 } & 718.8  & 719.6  & 731.9  & \textbf{720.0 } & 725.3  & 720.6  & 724.8  \\
    ...
    \multirow{3}[0]{*}{CEC4} & best   & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } & \textbf{1900.0 } \\
    ...
    \multirow{3}[0]{*}{CEC5} & best   & 2028.5  & 2119.2  & \textbf{1809.8 } & 1869.9  & 2435.3  & 2123.3  & 2048.8  & \textbf{1913.4 } & 2092.7  & 1970.9  \\
    ...
    \multirow{3}[0]{*}{CEC6} & best   & 1601.3  & 1601.0  & 1601.4  & 1601.5  & \textbf{1600.6 } & 1601.5  & 1601.2  & \textbf{1601.0 } & 1601.3  & 1601.2  \\
    ...
    \multirow{3}[0]{*}{CEC7} & best   & 2279.7  & \textbf{2120.7 } & 2202.0  & 2139.1  & 2234.9  & 2222.1  & 2146.2  & 2158.8  & 2281.3  & \textbf{2118.3 } \\
    ...
    \multirow{3}[0]{*}{CEC8} & best   & 2238.3  & 2230.6  & \textbf{2217.9 } & 2228.3  & 2301.0  & 2245.5  & 2226.2  & 2222.5  & \textbf{2211.4 } & 2234.4  \\
    ...
    \multirow{3}[0]{*}{CEC9} & best   & 2500.0  & 2500.0  & 2500.0  & 2500.0  & \textbf{2500.0 } & \textbf{2500.0 } & 2500.0  & 2500.0  & 2500.0  & \textbf{2500.0 } \\
    ...
    \multirow{3}[0]{*}{CEC10} & best   & 2897.7  & 2897.9  & 2897.9  & 2898.1  & \textbf{2600.1 } & 2600.2  & \textbf{2897.9 } & \textbf{2897.9 } & 2898.7  & \textbf{2897.9 } \\
...
\section{本章小结}
本章针对原始哈里斯鹰优化算法易陷入局部最优、后期收敛精度不足等问题, 本文提出一种多策略改进的哈里斯鹰优化算法。算法从三个层面进行增强: 引入经验互益策略, 利用历史信息提升跳出局部最优的能力; 设计能量自适应 Cauchy-Gaussian 扰动策略, 根据逃逸能量动态平衡全局探索与局部开发; 提出精英方差引导开发策略，依据精英方差调整搜索步长，提高后期收敛精度。与其他先进算法比较，IHHO 在 Friedman 平均排名 1.43、收敛速度与鲁棒性上均显著优于对比算法。进一步地, 将 IHHO 应用于 8 个高维生物医学数据集的特征选择问题，其在多数数据集上取得了更低的分类错误率与更高的准确率，证明了其在特征筛选中的有效性。综上所述，IHHO 有效克服了原始算法的缺陷，在连续优化与高维特征选择中均表现出优越性能, 为相关优化问题提供了更高效的解决方案。
\chapter{总结与展望}
