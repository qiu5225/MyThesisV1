...
\section{国内外研究现状}
优化的本质，是在各类理论研究与工程应用中，求解指定问题的最优解决方案。随着实际问题的复杂程度持续提升，近年来各领域对优化技术的要求也随之提高，特征选择问题同样遵循这一规律。
...
哈里斯鹰优化算法（HHO 作为一种新兴的元启发式算法，因其简单高效而备受关注。近年来，许多学者对其进行了改进并应用于不同领域。例如， 提出了一种基于混沌策略和准反向学习的改进 HHO 算法，用于解决工程设计问题，结果表明其在收敛速度和精度上均有显著提升。结合了正弦余弦算法（SCA）与 HHO，提出了一种混合优化算法，有效避免了局部最优，并在多目标优化问题中表现出色。 将 HHO 应用于光伏系统的参数辨识，通过引入自适应权重策略，提高了算法在复杂环境下的鲁棒性。此外， 提出了一种增强型 HHO 算法，利用莱维飞行和贪婪选择机制，成功应用于高维特征选择任务，验证了其在处理大规模数据时的有效性。尽管 HHO 在多个领域展现了潜力，但在处理复杂多模态问题时，仍存在收敛速度慢和易陷入局部最优的挑战，这为进一步的研究提供了空间。

尽管上述方法均取得良好效果，但仍存在诸多不足：例如参数设置复杂、泛化能力弱、鲁棒性差，且在高维数据集上性能不佳。特征选择是一项极具挑战性的优化任务，仍需持续的算法改进以实现更优性能。
NFL 定理 \cite {wolpert1997no} 从理论上证明：不存在可最优求解所有特征选择问题的通用算法。霜冰优化算法（RIME）是 Su 等人 \cite {su2023rime} 近期提出的元启发式算法。凭借极少的参数调优需求与稳定的优化能力，RIME 已获得广泛研究关注，并成功应用于图像处理与工程优化领域。例如，\cite {zhong2024srime} 融合拉丁超立方抽样提出增强型 RIME 算法（SRIME），用于工程优化任务；\cite {guo2024improved} 采用改进版 RIME 实现肺癌图像分割；\cite {hosney2024efficient} 将 RIME 与正交学习结合，应用于膀胱癌诊断；\cite {chen2025terime} 提出改进型 RIME 算法（TERIME），实现光伏模型的鲁棒参数提取；\cite {wang2025cgwrime} 在 RIME 中嵌入协作与竞争机制，提出 CGWRIME 算法并验证其工程优化效果；\cite {gu2024multi} 通过多策略改进 RIME，将其应用于三维无人水面艇（USV）路径规划；\cite {zheng2024stochastic} 将 RIME 与生物地理学算法混合，用于高精度图像分割。
...
\section{本文组织结构}
本文组织结构如下：第一章节阐述相关工作；第二章节节介绍特征选择问题与原始 RIME 算法；第三章节详细说明 IRIME 算法及其改进策略；第三章节详细说明改进的 HHO 算法及其改进策略；第五章节展示 CEC2017 测试集的完整实验结果并分析；第六章节节给出算法在特征选择任务中的应用结果与详细分析；最后在第七章节总结全文工作，并展望未来研究方向。
\chapter{相关背景知识介绍}
\section{特征选择问题}
\subsection{特征选择问题描述}
...
\subsection{EES 数学模型与阶段划分}
EES 策略将整个进化过程（最大迭代次数为 $T$）分为三个逻辑阶段：经验匮乏阶段（ESC）、经验交叉阶段（ECR）和经验共享阶段（ESH）。

\subsubsection{经验匮乏阶段 (Experience Scarcity Stage, ESC)}
...
\subsubsection{经验交叉阶段 (Experience Crossover Stage, ECR)}
...
\subsubsection{经验共享阶段 (Experience Sharing Stage, ESH)}
...
\subsection{Q-learning 核心概念}
Q-learning 的核心思想是学习一个状态-动作价值函数 $Q(s,a)$，该函数评估在状态 $s$ 下执行动作 $a$ 后预期的总回报。其更新公式为：
\begin{equation}
Q(s_t,a_t)\leftarrow Q(s_t,a_t)+\alpha\left[r_t+\gamma\max_{a^{\prime}}Q(s_{t+1},a^{\prime})-Q(s_t,a_t)\right]
\end{equation}
其中，$Q(s_t,a_t)$ 表示在状态 $s_t$ 下采取动作 $a_t$ 的当前估计价值；$\alpha$ 为学习率，控制更新步长；$\gamma$ 为折扣因子，代表未来回报的现值。

\subsection{在 HHO 中的应用}
在本研究中，我们将 HHO 的优化过程建模为一个马尔可夫决策过程 (MDP)。
\begin{itemize}
    \item \textbf{状态 ($S$)}：定义为当前种群的多样性水平或收敛程度。
    \item \textbf{动作 ($A$)}：定义为 HHO 算法中可选的策略，如不同的包围机制或经验交流策略的阶段切换。
    \item \textbf{奖励 ($R$)}：根据执行动作后种群最优适应度值的改善情况给予奖励。若适应度提升，给予正奖励；否则给予负奖励或零奖励。
\end{itemize}

通过引入 Q-learning，算法能够根据当前的搜索状态动态选择最合适的更新策略，从而在全局勘探和局部开发之间实现更智能的平衡。
