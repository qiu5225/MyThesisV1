...
\subsubsection{实验准备}
本实验选取7种先进算法，以及原始RIME算法作为对比算法。其余七种算法分别是：LSHADE、BDA、BWO、RSA、SCA、STOA和PDO。本实验所选取的算法及算法参数设置如表\ref{tb:alg-parameters-1}所示。本实验所涉及的CEC 2017数据集的相关信息在表\ref{tb:CEC2017-functions}中详细展示。实验的软、硬件环境在\ref{sec:exp-settings}节进行了详细介绍。
...
\subsection{特征选择应用}
本节旨在通过选择医学特征选择（FS）问题，验证IRIME在工程问题中的清晰度和有效性，以评估其优化性能。为确保全面评估，我们将所提方法与七种先进的元启发算法进行比较：RIME、AOA、BWO、WOA、ROA、SCA和COA。
\subsubsection{实验准备}
