...
哈里斯鹰优化算法（Harris Hawk Optimization, HHO）是一种模仿哈里斯鹰（Harris's hawks）合作狩猎行为的自然启发式元启发优化技术，于 2019 年由 Heidari 等人提出 \overcite {heidari2019harris}。该算法属于群体智能方法范畴，通过模拟哈里斯鹰的猎物包围、突袭及动态协作策略，构建了自适应的搜索机制，其核心优势在于能够动态平衡全局探索与局部开发过程，有效应对传统基于导数的优化器易陷入局部最优的问题，在复杂优化任务中展现出显著优越性。​
凭借其自适应优化特性，HHO 算法及其变体已在多领域实现广泛应用，展现出强劲的通用求解能力。在通信领域，HHO-PAP 变体通过联合功率分配和布局优化，显著提升了无人机辅助可见光通信系统的总速率，成功解决了无线通信中的非凸优化难题 \overcite {pham2020sum}；在电磁应用领域，HHO 算法为无线网络中的智能反射表面优化提供了高效解决方案 \overcite {xu2020intelligent}。在医疗相关的非特征选择场景中，HHO + 算法通过优化全 Elman 神经网络（FENN）的结构与参数，在心力衰竭监测任务中实现了 79% 的敏感性、78% 的准确率和 76% 的特异性，其增强的搜索空间探索能力超越了传统优化方法 \overcite {fetanat2021fully}；在医学影像处理领域，自适应 HHO（AHHO）算法通过动态调整探索开发平衡策略，在脑部 MRI 分割任务中树立了新标准，分割精度与效率均优于传统方法 \overcite {xu2020intelligent}。在方法学层面，HHO 算法的改进研究持续推进，包括结合仿生算法微调预训练模型、利用群体智能进行参数调优，以及通过 LLaMEA 等框架实现自动元启发演化等，进一步拓宽了其在计算社会科学等多领域的应用场景 \overcite {van2024llamea,de2021improving,alazab2024effective}。​
在高维医学特征选择这一关键场景中，已有部分学者基于 HHO 算法展开初步探索，为该领域应用奠定了基础。西安交通大学团队提出的增强型二进制哈里斯鹰优化算法，通过改进搜索策略强化特征筛选能力，其相关研究成果 “改进的哈里斯鹰优化算法及特征选择” 已获陕西省哲学社会科学优秀成果奖，验证了 HHO 算法在特征选择领域的理论价值与应用潜力 \overcite {3}。NSTL 收录的研究进一步证实，HHO 算法通过适配二元搜索空间特性，可有效降低高维数据中的特征冗余，为医学高维数据处理提供了新思路 \overcite {2}。在具体应用探索中，二元哈里斯鹰优化（BHHO）算法已被用于构建特征选择与集成学习的混合框架，通过 KNN 评估器完成特征筛选，为处理医学数据中常见的类别失衡与特征冗余问题提供了参考方案 \overcite {luthfi2025enhancing}。此外，混合 PSO-HHO 包裹式特征选择方法的提出，通过融合粒子群优化的全局搜索优势与 HHO 的局部开发能力，以欧氏距离 KNN 分类器筛选最优特征集，经 UCI 标准数据集验证，其分类准确率显著优于传统单一算法，为医学高维特征选择提供了可行的混合优化思路 \overcite {azeem2025new}。

尽管 HHO 算法在特征选择领域已展现出潜力，但现有研究在医学高维特征选择场景中仍存在明显短板：其一，专项研究严重匮乏，现有相关应用多为泛化性优化方法的迁移使用，未充分适配医学高维数据（如基因测序、多模态临床数据）的特殊性，缺乏针对医学特征关联性、噪声鲁棒性的专项改进；其二，计算复杂度问题突出，HHO 算法的迭代搜索过程在超高维数据处理中需消耗大量计算资源，而 HHOSSA 等混合改进方法进一步增加了处理时间与实现复杂度，严重制约临床应用效率 \overcite {issa2022efficient}；其三，离散化适配性不足，在 DHHO 等离散化实现中，动态网络结构与高维离散特征空间的适配矛盾导致算法性能衰减，易出现关键医学特征遗漏或冗余特征保留的问题 \overcite {khatri2023influence}；其四，探索与开发的动态平衡在高维空间中易失衡，导致算法收敛速度放缓、局部最优陷阱风险升高。这些问题的存在，严重限制了 HHO 算法在医学高维特征选择中的应用深度与广度。

为填补上述研究空白、克服算法固有缺陷，本文针对医学高维特征选择的核心需求，展开 HHO 算法的专项改进研究。通过融合多策略优化算法结构，一方面降低高维计算复杂度、提升搜索效率，另一方面强化医学数据适配性与特征筛选准确性，同时平衡探索与开发能力以规避局部最优，最终构建出适用于医学高维特征选择的高效优化算法，为临床诊断与医学研究提供更可靠的技术支撑。
...
\subsection{基准函数对比实验}
本节基于 CEC 2017 标准测试函数集开展全面对比实验，将本文提出的 IRIME 算法与八种主流先进优化算法进行综合性能评估。由于 F2 函数已被证实存在测试不稳定性，因此将其排除在本次实验之外。具有所有对比算法的参数配置列于表\ref{tb:alg-parameters-1}。研究涵盖三个关键分析维度：（1） 通过迭代曲线进行收敛行为分析。（2） 通过箱线图可视化进行解的稳定性评估。（3） 使用弗里德曼检验进行非参数统计验证。所有实验均在严格控制的条件下进行。
...
综合消融实验的整体测试结果可见，融合了全部三种改进策略的完整IHHO算法，依旧展现出卓越的综合优化竞争力。以函数求解平均值作为核心评价指标，IHHO算法在F1、F4、F7和F10测试函数上均取得最优求解结果，在F3、F8函数上也保持着极强的性能优势；从最优值指标分析，IHHO在F1、F3、F4、F7、F8、F9、F10等多个函数上均斩获最优值或并列最优值。上述结果充分证明，三种改进策略协同作用后，可有效驱动算法在全局探索能力、局部开发能力与求解结果稳定性之间实现高效平衡，充分发挥了多策略融合的增益效果。
...
总体而言，本文提出的三种改进策略功能定位清晰、互补性极强，不存在冗余重叠的优化环节。三者协同配合弥补了单一策略的性能短板，最终使得完整IHHO算法突破了单一改进策略的局限，获得了更均衡、更全面的综合优化性能。
...
表\ref{tb:IHHO-sensitivity-settings}列出了正交实验中各算法编号对应的参数组合。本文将$ph$、$L$、$elite\_ratio$和$\theta_0$作为待分析的关键参数，基于正交试验设计构建 9 组实验方案，依次编号为 IHHO1~IHHO9。该方法可在有效控制实验规模的前提下，精准剖析不同参数组合对算法求解性能的影响规律，同时为算法最优参数的确定提供实验依据。
